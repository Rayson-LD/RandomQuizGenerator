...
drive = input("Which drive would like to save the files in? ").upper()
try:
    os.chdir(f'{drive}:')
except Exception:
    print("Drive Does'nt Exist!")
    


print(os.listdir(f"{drive}:/"))
...
for quizNum in range(35): # Generate 35 quiz files.
    ...
    quizFile = open(f'{drive}:\{direc}\capitalsquiz{(quizNum + 1)}.txt', 'wt')
    answerKeyFile = open(f'{drive}:\{direc}\capitalsquiz_answers{(quizNum + 1)}.txt', 'wt')
